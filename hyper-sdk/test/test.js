...
function run(createTestSDKs, name) {
  const init = runOnFirstCall(async () => {
    ...
    cleanups.push(async () => {
      await Promise.all([close(), close2()]);
      await cleanup();
    });
  ...
  test(name + ': Hyperdrive - load drive over network', async (t) => {
    ...
    t.equal(await drive.readFile('/index.html', 'utf8'), EXAMPLE_DATA);
  });
  ...
  test(name + ': resolveName - resolve and load drive', async (t) => {
    ...
    t.equal(
      await new Promise((resolve, reject) =>
        resolveName(EXAMPLE_DNS_URL, (err, data) =>
  ...
  test(name + ': Hypercore - load from network', async (t) => {
    ...
    t.ok(await core2.get(0), 'got data from replicated core');
  });

  test(
    name + ': Hypercore - only close when all handles are closed',
    async (t) => {
      t.timeoutAfter(TEST_TIMEOUT);
      t.plan(5);
      ...
      t.ok(await core1.get(0), 'Still able to read after close');
      await core2.close();
